...
public class FighterEnemy : Ship
{
    ...
    void FollowPlayer()
    {
        Vector3 targetPos = (PlayerManager.Instance.player.isAlive ? player.position : (randomBuilding = randomBuilding ?? BuildingManager.Instance.GetRandomBuilding().gameObject).transform.position);
